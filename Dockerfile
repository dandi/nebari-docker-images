...
RUN --mount=type=cache,target=/opt/conda/pkgs,sharing=locked \
    --mount=type=cache,target=/root/.cache/pip,sharing=locked \
    /opt/scripts/install-conda-environment.sh /opt/dandi/environment.yaml 'false'

RUN df -h \
  && wget --quiet https://github.com/git-annex-remote-rclone/git-annex-remote-rclone/raw/refs/heads/master/git-annex-remote-rclone \
  && chmod +x git-annex-remote-rclone && mv git-annex-remote-rclone /opt/conda/bin

COPY dandi /opt/dandi
